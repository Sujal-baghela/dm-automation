...
export const InstagramAdapter: PlatformAdapter = {
  ...
  async publishPost(
    ...
    const creationId: string | undefined = createRes.id || createRes.creation_id || createRes.media_id

    if (!creationId) {
      throw new InstagramApiError("No creation id returned from Instagram", 500)
    }
    ...
    if (!mediaId) {
      throw new InstagramApiError("No media id returned from Instagram publish", 500)
    }
  ...
  async getPostAnalytics(postId: string, token: string): Promise<Record<string, number>> {
